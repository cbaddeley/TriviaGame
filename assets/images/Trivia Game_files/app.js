...
var pickedQuestionName;
var pickedQuestionButton;
var buttonArray = [" ", " ", " ", " "];

function questionCreator() {
  var pickedQuestionCounter = 0;
  while (!picked) {
    pickedQuestionNumber = Math.floor(Math.random() * questionArray.length);
...
function generateButtons(question) {
  ...
  var buttonArray = [" ", " ", " ", " "];
  buttonArray[answerButton] = $(this).question.name;
  while(!buttonPlacement) {
    ...
    if (wrongAnswerName !== $(this).question.name) {
      for (i = 0; i < buttonArray.length; i++) {
        if (wrongAnswerName === buttonArray[i]) {
          break;
        } 
      }
      var wrongButton = Math.floor(Math.random() * 4);
